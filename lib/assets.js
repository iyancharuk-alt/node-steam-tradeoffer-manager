...
TradeOfferManager.prototype._enrichOfferItemsAssetProperties = function(rawOffer, callback) {
	var partner = new SteamID('[U:1:' + rawOffer.accountid_other + ']');
	var groups = {};
	var self = this;

	var addItems = (steamid, items) => {
		var steamID64 = (typeof steamid === 'string' ? new SteamID(steamid) : steamid).getSteamID64();

		(items || []).forEach((item) => {
			if (!item || !item.appid || !item.contextid || !(item.assetid || item.id)) {
				return;
			}

			var key = steamID64 + '_' + item.appid + '_' + item.contextid;
			if (!groups[key]) {
				groups[key] = {
					steamid: steamID64,
					appid: item.appid,
	...
	if (rawOffer.is_our_offer) {
		addItems(this.steamID, rawOffer.items_to_give);
		addItems(partner, rawOffer.items_to_receive);
	} else {
		addItems(partner, rawOffer.items_to_give);
		addItems(this.steamID, rawOffer.items_to_receive);
	}

	var groupList = Object.keys(groups).map(key => groups[key]);
	if (!groupList.length) {
		callback(null);
		return;
	}

	Async.each(groupList, (group, done) => {
		var input = {
			...
			"for_trade_offer_verification": 1
		};

		if (self._language) {
			input.language = self._language;
		}

		group.items.forEach((item, index) => {
			input['filters[assetids][' + index + ']'] = (item.assetid || item.id).toString();
		});

		self._apiCall('GET', 'GetInventoryItemsWithDescriptions', 1, input, (err, body) => {
			if (!err && body && body.response && body.response.asset_properties) {
				var lookup = {};
				body.response.asset_properties.forEach((entry) => {
					if (entry.assetid && entry.asset_properties) {
						lookup[entry.assetid.toString()] = entry.asset_properties;
					}
				});

				group.items.forEach((item) => {
					var assetId = (item.assetid || item.id).toString();
					if (lookup[assetId]) {
						item.asset_properties = lookup[assetId];
					}
				});
			}
